...
void GreedySearch::initialize(Graph &graph) {
    goal = graph.get_goal();
...
bool GreedySearch::is_finished() {
    finished = (vector.empty() && !found);
    return finished;
}

bool GreedySearch::is_found() {
    found = (current == goal);
    return found;
}
...
void GreedySearch::step() {
    ...
    for (int i = 0; i < vector.size(); i++) {
        ...
        int distance = manhattan_distance(node, goal);
    ...
    std::cout << current->get_cost() << std::endl;
    ...
    for (int i = 0; i < child_count; i++) {
        ...
        if (!visited.contains(child_node)) {
            child_node->set_cost(manhattan_distance(child_node, goal));
